...
const REVIEW_SCHEMA = {
  ...
  properties: {
    ...
    projectRelation: { type: 'string', enum: ['same-project', 'uncertain', 'different-project'] },
    archiveShape: { type: 'string', enum: ['full-snapshot', 'patch-like', 'unknown'] },
    recommendation: { type: 'string', enum: ['continue', 'manual-review', 'reinterpret-as-overlay', 'cancel-update'] },
  },
  required: ['assessment', 'confidence', 'reasons', 'projectRelation', 'archiveShape', 'recommendation'],
};
...
function deletionIntentSystemPrompt(promptLang, outputLanguage) {
  return [
    ...
    'Return the requested structured fields. If the transport cannot enforce JSON schema, return plain text without Markdown fences using exactly these headings:',
...
function systemPrompt(promptLang, outputLanguage) {
  return [
    ...
    'PROJECT RELATION: followed by same-project, uncertain, or different-project.',
    'ARCHIVE SHAPE: followed by full-snapshot, patch-like, or unknown.',
    'RECOMMENDATION: followed by continue, manual-review, reinterpret-as-overlay, or cancel-update.',
    'Use cancel-update whenever the archive appears to belong to a different project. Changing snapshot mode cannot make an unrelated archive safe.',
    'Use reinterpret-as-overlay only when the archive belongs to this project but looks like a partial patch that omits unchanged files.',
    'Use continue only for a plausible same-project archive. Use manual-review when project identity or archive shape remains uncertain.',
    'CONFIDENCE: followed by low, medium, or high.',
