...
import { verifyBackupFiles } from './backup.js';

export async function inspectRollback(runId, { signal = null } = {}) {
  ...
  await verifyBackupFiles({ root, manifest, signal });
  const changedAfter = [];
...
export async function rollbackRun(runId, { onProgress = null, signal = null } = {}) {
  const inspection = await inspectRollback(runId, { signal });
  ...
  for (const item of [...manifest.items].reverse()) {
    ...
    if (!item.existed) {
      ...
      try {
        ...
        if (item.mode !== null && item.mode !== undefined) await chmod(temporary, item.mode);
...
async function validateBackupManifest(runId, root, manifest) {
  if (![1, 2].includes(manifest.version) || manifest.runId !== runId || !Array.isArray(manifest.items)) {
  ...
  if (![1, 2].includes(binding?.version) || binding.runId !== runId || !binding.projectPath) {
  ...
  for (const item of manifest.items) {
    ...
    if (manifest.version >= 2 && item.existed && !item.backupHash) throw unsafeBackup(`Backup manifest is missing the verified hash for ${relative}.`);
  }
}
