...
`src/ui/rich-text.js` separates ordinary text, fenced code, and standalone JSON without changing the stored source text. `src/ui/syntax-render.js` uses the documented Terlio 1.2.0 `highlightSyntaxLines`, `highlightSyntax`, and `SyntaxText` surfaces in that order, adapting their output into Activity lines with a plain-text fallback. Activity, live LLM output, saved raw responses, and model replay all use this shared path.
...
- Terlio 1.2.0 fenced-code and streaming/standalone JSON syntax routing;
