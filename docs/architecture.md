...
The active pack is represented in application state so rendering remains deterministic and cache signatures can include the language. Shared workflow chrome, setup menus, settings, context help, Activity, replay workspaces, statuses, and toasts route through one translation boundary. The English pack is the canonical complete catalog. Source checks audit static user-facing fields and common status/toast calls against English, reject translation keys absent from English, the Russian regression audit verifies complete bundled Russian coverage, and concrete baseline assertions protect the core German, French, Italian, and Spanish catalogs. Overlapping patterns are ordered from most specific to least specific so a compact prefix translation cannot consume a richer statistic or help line. Missing strings fall back to the built-in English catalog, allowing third-party packs to be incremental while keeping every screen usable. Changing the language reconfigures the registry atomically and rerenders without rewriting project workflows.
...
Uses provider-specific adapters. LM Studio model metadata comes from the native `/api/v1/models` endpoint, including parameter counts and loaded-instance configuration. `settings-model.js` persists optional per-model load parameters, unloads stale LLM instances, and invokes `/api/v1/models/load` to make the selected configuration active; generation uses only the native `/api/v1/chat` stream, including model-load, prompt-processing, reasoning, message, and error events. Ollama model discovery uses `/api/tags`, context metadata comes from `/api/ps` and `/api/show`, and generation uses the native `/api/chat` NDJSON stream. The generic OpenAI-compatible adapter discovers IDs through a configurable `/models` endpoint and supports Responses API SSE, Chat Completions SSE/JSON, and endpoint fallback. The Codex adapter supports stdio JSONL plus WebSocket-framed JSON-RPC over TCP or Unix sockets. The default shared Unix control socket is internal and is not shown while managed mode is selected. It is probed with a complete initialize/initialized handshake and reused when compatible; only an unavailable managed default causes Zipflow to start one detached local listener. Enabling the external-server switch reveals the editable endpoint, and every custom endpoint is connect-only. The adapter discovers models through `model/list`, creates ephemeral read-only threads, and requires a successful `turn/completed` notification. Loaded LM Studio instances are resolved before generation and addressed by instance ID. Model metadata, the resolved instance, context configuration, and bearer token are captured once per LLM run and reused by structure review, patch analysis, chunk synthesis, and repair requests; a successful generation is never invalidated by a later metadata refresh. When a loaded instance is selected, generation addresses that instance directly and does not repeat `context_length`; this reuses the same allocation for reviews and compatibility tests instead of asking LM Studio to create another instance. Load-time settings are sent only when a model is not already loaded. API tokens are applied only as authorization headers and are never emitted through Activity events. An `AbortSignal` is threaded through metadata, HTTP requests, SSE/NDJSON consumption, and Codex RPC turns so `Esc` cancels only LLM generation while the archive plan continues. Every streamed provider must emit its own successful terminal state; transport EOF alone is classified as incomplete generation.
...
`src/ui/rich-text.js` separates ordinary text, fenced code, and standalone JSON without changing the stored source text. `src/ui/syntax-render.js` uses the documented Terlio 1.2.1 `highlightSyntaxLines`, `highlightSyntax`, and `SyntaxText` surfaces in that order, adapting their output into Activity lines with a plain-text fallback. Activity, live LLM output, saved raw responses, and model replay all use this shared path.
...
Global settings use a two-panel category/detail state machine. The left panel contains stable top-level categories without descriptions. The right panel displays concise `Parameter: value` rows and section headings; only actionable rows receive focus. Archive, backup, and managed-history statistics are projected into one compact summary row beneath the selected parameter help, so storage information remains visible without reintroducing multi-line menu items. A fixed two-row context dock appears below the focused list, clipping long context instead of changing geometry; full help remains available in a native blocking Terlio help overlay with background dimming and shadow. Transient notifications remain stored in Terlio's overlay manager but are rendered by Zipflow's adaptive toast overlay. It derives a bounded width from the message, wraps every detail line, preserves the final text, supports dismissal, and keeps the existing shadow and semantic severity styling. `Tab` switches pane focus, `Enter` or `Space` activates, and returning restores prior category, parameter, and value positions. Dependent controls are omitted or disabled with an explicit reason, and input-like values open in a modal without replacing either panel.
...
- LM Studio and Ollama native metadata, OpenAI-compatible and Codex app-server model discovery, Codex permission-profile discovery with fail-closed `:read-only` selection, atomic model selection, unload/reload configuration, configurable API transport and reasoning effort, separate prompt/summary/commit languages, compatibility tests, read-only historical replay, context budgeting, structural patch truncation, adaptive/full/path-list/file-batch delivery, optional authorization, SSE and NDJSON streams, readable wrapped progress, context/OOM retries, reasoning-only responses, hidden repair requests, failure explanations, and diagnostics;
...
- Terlio 1.2.1 fenced-code and streaming/standalone JSON syntax routing, compact/default and explicit block progress behavior, tall block-glyph repaint priority, and normalized Shift-modified printable keys;
